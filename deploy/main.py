...
# Flaskインスタンス生成時にstatic_folderを明示
...
def try_create_manual_pdf(plan_id):
    ...
    if parts_manual_exists and assembly_manual_exists:
        try:
...
def get_assembly_procedure(plan_id, procedure_no):
    ...
    if not filtered_parts3d:
        logging.warning(
            f"[get_assembly_procedure] No parts3d found for procedure {procedure_no} in {plan_id}"
        )
        obj_text = ""
